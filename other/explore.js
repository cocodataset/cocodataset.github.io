...
$(window).scroll(function() {
    if ($(window).scrollTop() >= $(document).height() - $(window).height() - 10) {
        if ($('#btn-search').prop('disabled') == false) {
...
function createDisplay(imageId, captions, catToSegms, flickrUrl, cocoUrl) {
    ...
    var cocoURL = '<a href="explore.htm?id=' + imageId + '" target="_blank">' + 'http://cocodataset.org/explore.htm?id=' + imageId + '</a>';
...
$(function() {
    $("#tags").tagit({
        ...
        afterTagAdded: function(event, ui){
            if (iconNameList.indexOf(ui.tagLabel) != -1 || !isNaN(ui.tagLabel)){
                addTags(ui.tagLabel);
            }else{
...
function setupIconsSearchPanel(){
    ...
    for (i=0; i<icons.length; i++){
        ...
        iconNameList.push(icon.attr('title'));
    }
}
...
function loadIconPanel(){
    var iconGroup = $('#exploreIconPanel');
    var superCats = ['accessory', 'vehicle', 'outdoor', 'animal', 'sports', 'kitchen',  'food', 'furniture', 'electronic', 'appliance', 'indoor'];
    var iconSubPanel = '';
    for(var i=0; i<superCats.length; i++){
        iconSubPanel += '<div style="width:70px;height:150px; display:inline-block;vertical-align:top; margin-left:2px">';
        var cats = categories[superCats[i]];
        for(var j=0; j<cats.length; j++){
            var cat = cats[j];
            iconSubPanel += '<div id="icon_'+cat.id+'" class="icons" title="'+cat.name+'" style="margin-right:-2px;margin-bottom:-2px;width:36px;height:36px;display:inline-block;vertical-align:top; border-color:transparent;border-style:solid;border-width:3px;cursor:pointer;"><img src="images/icons/'+cat.id+'.jpg" style="width:30px;height:30px"> </div>';
        }
        iconSubPanel += '</div>';
    }
    iconGroup.append(iconSubPanel);
}